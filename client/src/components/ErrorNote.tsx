...
function explain(error: ApiError) {
  ...
  if (error.code === 'VERSION_CONFLICT') {
    return (
      <p>
        Someone else moved this file while you had it open. Reload to see where
        it actually is before trying again.
      </p>
    )
  }
  ...
  return (
    <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
      {error instanceof ApiError ? (
        ...
        <p>
          Could not reach the API. If it has been quiet for a while the server
          may be waking up — give it a minute and try again.
        </p>
      )}
    </div>
  )
